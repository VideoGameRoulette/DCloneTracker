...
app.get('/api/d2rw/ladder/softcore', async (req, res) => {
  const data = await axios.get(
    'https://d2runewizard.com/api/diablo-clone-progress/ladder/softcore',
  );
  if (data.status < 400) {
...
app.get('/api/d2rw/ladder/hardcore', async (req, res) => {
  const data = await axios.get(
    'https://d2runewizard.com/api/diablo-clone-progress/ladder/hardcore',
  );
  if (data.status < 400) {
    return res.json({ data: data.data.servers });
  }
...
app.get('/api/d2rw/nonladder/softcore', async (req, res) => {
  const data = await axios.get(
    'https://d2runewizard.com/api/diablo-clone-progress/nonLadder/softcore',
  );
  if (data.status < 400) {
    return res.json({ data: data.data.servers });
  }
...
app.get('/api/d2rw/nonladder/hardcore', async (req, res) => {
  const data = await axios.get(
    'https://d2runewizard.com/api/diablo-clone-progress/nonLadder/hardcore',
  );
  if (data.status < 400) {
    return res.json({ data: data.data.servers });
  }
